...
const GS = `
  ...
  *, *::before, *::after { margin:0; padding:0; box-sizing:border-box; }
  html, body { background: #060d1a; color: #fff; font-family: 'DM Sans', 'Segoe UI', sans-serif; -webkit-font-smoothing: antialiased; }
...
const PrimaryBtn = ({ children, onClick, disabled=false, style={} }) => (
  <button onClick={onClick} disabled={disabled}
    style={{ background:`linear-gradient(135deg,${C.blue},${C.blueBtn})`, border:"none", color:C.white, padding:"16px 24px", borderRadius:13, fontFamily:F.sans, fontSize:15, fontWeight:600, cursor:disabled?"not-allowed":"pointer", width:"100%", boxShadow:"0 8px 32px rgba(74,143,212,0.25)", transition:"all .22s", opacity:disabled?0.5:1, ...style }}
  ...
  <div className="ov" style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.78)", zIndex:999, display:"flex", alignItems:"flex-end", justifyContent:"center" }}>
    <div className="mod" style={{ background:C.bgCard, borderRadius:"20px 20px 0 0", padding:"28px 24px 36px", maxWidth:480, width:"100%", border:`1px solid ${C.border}`, maxHeight:"85vh", display:"flex", flexDirection:"column" }}>
      ...
      <div style={{ display:"flex", gap:10 }}>
        ...
        <button onClick={onAccept} style={{ flex:2, background:`linear-gradient(135deg,${C.blue},${C.blueBtn})`, border:"none", color:C.white, padding:14, borderRadius:10, fontFamily:F.sans, fontSize:14, fontWeight:600, cursor:"pointer" }}>Acepto los términos ✓</button>
      </div>
    </div>
  </div>
);
    ...
    <div className="ov" style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.78)", zIndex:999, display:"flex", alignItems:"flex-end", justifyContent:"center" }}>
      <div className="mod" style={{ background:C.bgCard, borderRadius:"20px 20px 0 0", padding:"28px 24px 36px", maxWidth:480, width:"100%", border:`1px solid ${C.border}` }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
          <h3 style={{ fontFamily:F.serif, fontSize:22 }}>Registrate para continuar</h3>
          <button onClick={onClose} style={{ background:"none", border:"none", color:C.gray, fontSize:22, cursor:"pointer" }}>×</button>
        </div>
        <p style={{ color:C.gray, fontSize:13, marginBottom:20, fontWeight:300 }}>Necesitamos saber quién sos para que los profesionales puedan contactarte.</p>
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          <div style={{ background:"rgba(74,143,212,0.06)", border:`1px solid ${C.border}`, borderRadius:10, padding:"10px 14px", fontSize:12, color:C.gray }}>
  ...
  return (
    <div className="chat" style={{ position:"fixed", inset:0, background:C.bg, zIndex:200, display:"flex", flexDirection:"column", maxWidth:480, margin:"0 auto" }}>
      <div style={{ background:C.nav, backdropFilter:"blur(12px)", borderBottom:`1px solid ${C.border}`, padding:"14px 20px", display:"flex", alignItems:"center", gap:14 }}>
        ...
        <div style={{ width:40, height:40, borderRadius:10, fontSize:20, background:"rgba(74,143,212,0.12)", display:"flex", alignItems:"center", justifyContent:"center" }}>{pro.img}</div>
        <div style={{ flex:1 }}>
          <div style={{ fontWeight:600, fontSize:15 }}>{pro.nombre} {pro.verificado&&<span style={{fontSize:10,color:C.blue,background:"rgba(74,143,212,0.12)",padding:"2px 6px",borderRadius:20}}>✓</span>}</div>
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"20px 16px", display:"flex", flexDirection:"column", gap:10 }}>
        {msgs.map(m=>(
          <div key={m.id} style={{ display:"flex", flexDirection:"column", alignItems:m.de==="cliente"?"flex-end":"flex-start" }}>
            <div style={{ maxWidth:"78%", padding:"11px 14px", borderRadius:m.de==="cliente"?"16px 16px 4px 16px":"16px 16px 16px 4px", background:m.de==="cliente"?`linear-gradient(135deg,${C.blue},${C.blueBtn})`:C.bgCard, border:m.de==="pro"?`1px solid ${C.border}`:"none", fontSize:14, lineHeight:1.55 }}>{m.texto}</div>
      ...
      <div style={{ background:C.nav, borderTop:`1px solid ${C.border}`, padding:"14px 16px", display:"flex", gap:10, alignItems:"flex-end" }}>
        <textarea value={texto} onChange={e=>setTexto(e.target.value)} onKeyDown={e=>{ if(e.key==="Enter"&&!e.shiftKey){ e.preventDefault(); send(); }}} placeholder="Escribí un mensaje..." rows={1}
          style={{ flex:1, background:C.bgCard, border:`1px solid ${C.border}`, color:C.white, padding:"12px 16px", borderRadius:12, fontFamily:F.sans, fontSize:14, resize:"none", lineHeight:1.5, maxHeight:100 }}/>
        <button onClick={send} disabled={!texto.trim()} style={{ width:46, height:46, borderRadius:12, background:texto.trim()?`linear-gradient(135deg,${C.blue},${C.blueBtn})`:"rgba(74,143,212,0.1)", border:"none", cursor:texto.trim()?"pointer":"default", fontSize:20, flexShrink:0 }}>➤</button>
      </div>
    </div>
  );
};
    ...
    <div style={{ minHeight:"100vh", background:C.bg, maxWidth:480, margin:"0 auto" }}>
      ...
      {vista==="inicio" && (
        <div>
          <div style={{ minHeight:"88vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"40px 28px 60px", background:"radial-gradient(ellipse at 50% 0%, rgba(74,143,212,0.12) 0%, transparent 65%)" }}>
            ...
            {/* ✅ TÍTULO EN BLANCO */}
            <h1 className="fu1" style={{ fontFamily:F.serif, fontSize:"clamp(40px,8vw,60px)", fontWeight:700, lineHeight:1.12, textAlign:"center", marginBottom:20, color:"#ffffff" }}>
          ...
          <div style={{ padding:"0 24px 40px" }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20 }}>
              <div>
                ...
                <p style={{ color:C.gray, fontSize:13, marginTop:4, fontWeight:300 }}>10 categorías · 80+ oficios</p>
              </div>
          ...
          <div style={{ padding:"0 24px 60px" }}>
            ...
            {activos.map(p=>(
              ...
              >
                <div style={{ width:46, height:46, borderRadius:12, fontSize:22, background:"rgba(74,143,212,0.1)", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>{p.img}</div>
                <div style={{ flex:1 }}>
                  <div style={{ fontWeight:600, fontSize:15 }}>{p.nombre} {p.verificado&&<span style={{fontSize:11,color:C.blue,background:"rgba(74,143,212,0.12)",padding:"2px 7px",borderRadius:20}}>✓</span>}</div>
      ...
      {vista==="categorias" && (
        <div className="page" style={{ padding:"24px 20px 60px" }}>
          ...
          <p style={{ color:C.gray, fontSize:13, marginBottom:20, fontWeight:300 }}>10 categorías · 80+ profesiones</p>
            ...
            <div>
              ...
              <div style={{ display:"flex", gap:8, marginTop:8 }}>
                {["Urgente (hoy)","Esta semana","Sin apuro"].map((u,i)=>(
                  <button key={u} style={{ flex:1, padding:"10px 4px", background:i===0?"rgba(74,143,212,0.15)":C.bgCard, border:`1px solid ${i===0?C.blue:C.border}`, borderRadius:8, color:i===0?C.blueLight:C.gray, fontSize:11, fontFamily:F.sans, cursor:"pointer" }}>{u}</button>
                ))}
              </div>
            </div>
          ...
          {activos.slice(0,3).map((p,i)=>(
            <div key={p.id} style={{ background:C.bgCard, border:`1px solid ${C.border}`, borderRadius:14, padding:18, marginBottom:12 }}>
              ...
              <div style={{ display:"flex", gap:8 }}>
                <button onClick={()=>setVista("perfil")} style={{ flex:1, background:`linear-gradient(135deg,${C.blue},${C.blueBtn})`, border:"none", color:C.white, padding:11, borderRadius:10, fontFamily:F.sans, fontSize:13, fontWeight:600, cursor:"pointer" }}>Ver perfil</button>
        ...
        <div className="page">
          ...
          <div style={{ padding:"0 24px 60px" }}>
            ...
            <div style={{ display:"flex", gap:8 }}>
              ...
              <button onClick={()=>setShowCalif(true)} style={{ flex:1, background:"transparent", border:`1px solid ${C.border}`, color:C.grayLight, padding:14, borderRadius:13, fontFamily:F.sans, fontSize:14, cursor:"pointer" }}>★ Calificar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {vista==="notificaciones" && (
        <div className="page" style={{ padding:"24px 20px 60px" }}>
          ...
          <p style={{ color:C.gray, fontSize:13, marginBottom:20, fontWeight:300 }}>Pedidos nuevos en tu rubro</p>
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {NOTIFICACIONES.map(n=>{
              ...
              return (
                <div key={n.id} onClick={()=>!postulado&&abrirNotif(n)}
                  ...
                  <div style={{ color:C.gray, fontSize:13, lineHeight:1.5 }}>{n.desc}</div>
                </div>
              );
            })}
          </div>
      ...
      {vista==="planes" && (
        <div className="page" style={{ padding:"32px 24px 60px" }}>
          ...
          <p style={{ color:C.gray, fontSize:14, marginBottom:36, fontWeight:300 }}>Todos los planes incluyen verificación de identidad ✓</p>
          {[
            ...
            { nombre:"Empresa",     precio:"$8.000",  sub:"por mes",           dest:false, tag:"PARA EMPRESAS",features:["Todo Profesional","Hasta 5 usuarios del equipo","Primero en búsquedas","Estadísticas avanzadas","Gestión de agenda","Panel de empresa","Facturación integrada"] },
          ].map(p=>(
            <div key={p.nombre} style={{ background:p.dest?"linear-gradient(135deg,rgba(74,143,212,0.18),rgba(30,77,130,0.18))":C.bgCard, border:`1px solid ${p.dest?C.blue:p.nombre==="Empresa"?"rgba(212,180,74,0.4)":C.border}`, borderRadius:16, padding:24, position:"relative", boxShadow:p.dest?"0 0 40px rgba(74,143,212,0.15)":"none", marginBottom:16 }}>
              {p.tag && <div style={{ position:"absolute", top:-12, left:"50%", transform:"translateX(-50%)", background:p.dest?C.blue:"rgba(212,180,74,0.9)", color:C.white, fontSize:11, fontWeight:600, padding:"4px 16px", borderRadius:20, letterSpacing:1, whiteSpace:"nowrap" }}>{p.tag}</div>}
              ...
              <button style={{ width:"100%", background:p.dest?`linear-gradient(135deg,${C.blue},${C.blueBtn})`:p.nombre==="Empresa"?"rgba(212,180,74,0.15)":"transparent", border:p.dest?"none":p.nombre==="Empresa"?"1px solid rgba(212,180,74,0.4)":`1px solid ${C.border}`, color:p.dest?C.white:p.nombre==="Empresa"?C.yellow:C.grayLight, padding:14, borderRadius:10, fontFamily:F.sans, fontSize:14, fontWeight:600, cursor:"pointer" }}>
                {p.nombre==="Inicio"?"Comenzar gratis":p.nombre==="Empresa"?"Contactar a Nexo":`Elegir ${p.nombre}`}
              </button>
            </div>
          ))}
        </div>
          ...
          <div style={{ display:"flex", gap:6, marginBottom:20, background:C.bgCard, padding:4, borderRadius:10, border:`1px solid ${C.border}` }}>
            {[{id:"verificaciones",label:"Verificaciones"},{id:"profesionales",label:"Profesionales"},{id:"pedidos",label:"Pedidos"}].map(tab=>(
              <button key={tab.id} onClick={()=>setAdminTab(tab.id)}
                style={{ flex:1, padding:"9px 6px", borderRadius:8, border:"none", background:adminTab===tab.id?`linear-gradient(135deg,${C.blue},${C.blueBtn})`:"transparent", color:adminTab===tab.id?C.white:C.gray, fontFamily:F.sans, fontSize:12, fontWeight:600, cursor:"pointer", whiteSpace:"nowrap" }}>
                {tab.label}
              </button>
            ))}
          </div>
              ...
              {pendientes.length===0 ? (
                ...
                <div key={p.id} style={{ background:C.bgCard, border:`1px solid ${C.yellow}33`, borderRadius:14, padding:18, marginBottom:12 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:14 }}>
                    <div style={{ width:46, height:46, borderRadius:10, fontSize:22, background:"rgba(74,143,212,0.1)", display:"flex", alignItems:"center", justifyContent:"center" }}>{p.img}</div>
                    <div style={{ flex:1 }}>
                      <div style={{ fontWeight:600, fontSize:15 }}>{p.nombre} <span style={{ fontSize:10, color:C.yellow, background:"rgba(251,191,36,0.12)", padding:"2px 8px", borderRadius:20 }}>PENDIENTE</span></div>
                      <div style={{ color:C.gray, fontSize:12, marginTop:2 }}>{p.oficio} · {p.zona}</div>
                    </div>
                  </div>
                  ...
                  <div style={{ marginBottom:12 }}>
                    ...
                    <textarea value={adminMsg[p.id]||""} onChange={e=>setAdminMsg(prev=>({...prev,[p.id]:e.target.value}))} placeholder="Mensaje opcional..."
                      style={{ width:"100%", marginTop:8, background:C.bg, border:`1px solid ${C.border}`, color:C.white, padding:"11px 14px", borderRadius:10, fontFamily:F.sans, fontSize:13, resize:"none", minHeight:56 }}/>
                  </div>
                  <div style={{ display:"flex", gap:8 }}>
